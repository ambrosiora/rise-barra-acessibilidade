...
function HTMLMagnifier(options) {
    ...
    function syncViewport() {
      ...
      // como tava antes o código, mas não tava funcionando retornando 0
    ...
    //   const y2 = document.body.scrollTop;
      ...
      const x2 = window.pageXOffset || document.documentElement.scrollLeft || document.body.scrollLeft || 0;
      const left = -x1 * _this.options.zoom - x2 * _this.options.zoom;
      const top = -y1 * _this.options.zoom - y2 * _this.options.zoom;
      //foi colocado - 70 para ficar centralizado o top do zoom
      setPosition(magnifierContent, left - 175, top - 70);
    ...
    function makeDraggable(ctrl, options) {
      ...
      function moveHandler(e) {
        if (dragObject !== null) {
          ...
          const screenWindth = this.innerWidth - 80;
          const screenHeight = this.innerHeight - 80;

          if((-185 <= left && screenWindth > left) && (-70 < top && screenHeight > top)) {
